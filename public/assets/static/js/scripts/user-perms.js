$(function () {
    ...
    $('.modal-effect').on('click', function (e) {
        ...
        $.get('/backend/user-permissions/' + $(this).attr('data-id'))
            .done(function (request) {
                if (request != undefined) {

                //Load data then launch modal.
                $('#modal-edit').find('#modal-edit-body').html(request);
                $('#modal-edit').modal('show');
                $('#modal-edit').find('.select2-multi').select2({
                    minimumResultsForSearch: Infinity,
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/user-permissions/' + $(this).find('input[name="id"]').val(), $(this).serialize())
                .done(function (data) {
                    // console.log(data);return false;
                    try {
                        if (data.status == 'success') {
                            $('.modal').find('#modal-edit-msg').html('');
                            setTimeout(function () {
                ...
                .fail(function (jqXHR, textStatus, errorThrown) {
                    if (jqXHR.status == 422) {
                        ...
                        $('.modal').find('#modal-edit-msg').html(alerts('danger', errors));
                    } else {
                        swal('Erreur!', 'Operation echoué, un erreur survenue !', 'error');
                    }
                })
    ...
    $('#form-add').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/user-permissions', $(this).serialize() + '&status=' + Number($('.br-toggle').hasClass('on')))
                .done(function (data) {
                    try {
                        if (data.status == 'success') {
                            $('#modal-add-msg').html('');
    ...
    $('.btn-status').on('click', function (e) {
        ...
        $.post('/backend/user-permissions/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/user-permissions/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
