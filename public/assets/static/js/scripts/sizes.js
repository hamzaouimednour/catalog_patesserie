$(function () {
    ...
    $('.table').on('click', '.modal-effect', function (e) {
        ...
        $.getJSON('/backend/sizes/' + $(this).attr('data-id'))
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/sizes/' + $(this).find('input[name="id"]').val(), $(this).serialize())
                .done(function (data) {
                    try {
    ...
    $('#form-add').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/sizes', $(this).serialize() + '&status=' + Number($('.br-toggle').hasClass('on')))
                .done(function (data) {
                    try {
                        if (data.status == 'success') {
                            $('#modal-add-msg').html('');
    ...
    $('.table').on('click', '.btn-status',function (e) {
        ...
        $.post('/backend/sizes/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/sizes/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
