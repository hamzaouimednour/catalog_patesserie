$(function () {
    ...
    $('.modal-effect').on('click', function (e) {
        ...
        $.getJSON('/backend/components/' + $(this).attr('data-id'))
    ...
    $('#modal-edit, #modal-add').on('hidden.bs.modal', function (e) {
        ...
        $('#file-a-label, #file-e-label').html('Choisir Fichier');
    });
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.ajax({
                url: '/backend/components/' + $(this).find('input[name="id"]').val(),
                type: "POST",
    ...
    $('#field-a-upload, #field-e-upload').on('change', function (e) {
        // alert(jQuery(this).val())
        if ($(this)[0].files.length){
            $('#file-a-label, #file-e-label').html($(this)[0].files[0].name);
        }
    })
    // click on status btn
    $('.btn-status').on('click', function (e) {
        ...
        $.post('/backend/components/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/components/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
