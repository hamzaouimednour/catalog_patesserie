$(function () {
    ...
    $('.modal-effect').on('click', function (e) {
        ...
        $.getJSON('/backend/users/' + $(this).attr('data-id'))
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/users/' + $(this).find('input[name="id"]').val(), $(this).serialize())
                .done(function (data) {
    ...
    $('#form-add').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/users', $(this).serialize() + '&status=' + Number($('.br-toggle').hasClass('on')))
                .done(function (data) {
                    try {
                        if (data.status == 'success') {
                            $('#modal-add-msg').html('');
    ...
    $('.btn-status').on('click', function (e) {
        ...
        $.post('/backend/users/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/users/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
