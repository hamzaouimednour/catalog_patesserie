$(function () {
    ...
    $('.table').on('click', '.modal-effect', function (e) {
        ...
        $.getJSON('/backend/sub-tags/' + $(this).attr('data-id'))
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.ajax({
                url: "/backend/sub-tags/" + $(this).find('input[name="id"]').val(),
                type: "POST",
                data: formData,
                contentType: false,
                cache: false,
    ...
    $('.table').on('click', '.btn-status', function (e) {
        ...
        $.post('/backend/sub-tags/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/sub-tags/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
