$(function () {
    ...
    $('.modal-effect').on('click', function (e) {
        ...
        $.getJSON('/backend/company-users/' + $(this).attr('data-id'))
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/company-users/' + $(this).find('input[name="id"]').val(), $(this).serialize())
                .done(function (data) {
                    try {
    ...
    $('#form-add').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/company-users', $(this).serialize() + '&status=' + Number($('.br-toggle').hasClass('on')))
                .done(function (data) {
                    console.log(data)
                    try {
                        if (data.status == 'success') {
    ...
    $('.btn-status').on('click', function (e) {
        ...
        $.post('/backend/company-users/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/company-users/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
