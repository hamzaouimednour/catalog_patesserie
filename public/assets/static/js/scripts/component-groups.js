$(function () {
    ...
    $('.modal-effect').on('click', function (e) {
        ...
        $.getJSON('/backend/component-groups/' + $(this).attr('data-id'))
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/component-groups/' + $(this).find('input[name="id"]').val(), $(this).serialize())
                .done(function (data) {
                    try {
    ...
    $('#form-add').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/component-groups', $(this).serialize() + '&status=' + Number($('.br-toggle').hasClass('on')))
                .done(function (data) {
                    try {
                        if (data.status == 'success') {
    ...
    $('.btn-status').on('click', function (e) {
        ...
        $.post('/backend/component-groups/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
        ...
        swal({
            ...
            .then((willAccept) => {
                if (willAccept) {
                    ...
                    $.post('/backend/component-groups/delete/' + item_id)
                        .done(function (data) {
                            try {
                                if (data.status === 'success') {
