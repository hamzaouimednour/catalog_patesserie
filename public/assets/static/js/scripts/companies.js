$(function () {
    ...
    $('.modal-effect').on('click', function (e) {
        ...
        $.getJSON('/backend/companies/' + $(this).attr('data-id'))
    ...
    $('#form-edit').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/companies/' + $(this).find('input[name="id"]').val(), $(this).serialize())
                .done(function (data) {
                    try {
                        if (data.status == 'success') {
                            $('#modal-edit-msg').html(alerts('success', 'Opération terminé avec succés.', 'success'));
                            // hide modal event
                            $('#modal-edit').on('hide.bs.modal', function (e) { location.reload(); });
                        } else {
    ...
    $('#form-add').submit(function (e) {
        ...
        if ($(this)[0].checkValidity() === false) {
            ...
            $.post('/backend/companies', $(this).serialize())
                .done(function (data) {
                    try {
                        if (data.status == 'success') {
                            $('#modal-add-msg').html(alerts('success', 'Opération terminé avec succés.', 'success'));
                            // hide modal event
                            $('#modal-add').on('hide.bs.modal', function (e) { location.reload(); });
                        } else {
    ...
    $('.btn-status').on('click', function(e) {
        ...
        var req = { status: new_status[old_status], id: $(this).attr('data-id') };
        $.post('/backend/companies/update/status', req)
            .done(function (data) {
                try {
                    if (data.status == 'success') {
            ...
            buttons: {
                ...
                .then((willAccept) => {
                    if (willAccept) {
                        ...
                        $.post('/backend/companies/delete/' + item_id)
                            .done(function (data) {
                                try {
                                    if (data.status === 'success') {
