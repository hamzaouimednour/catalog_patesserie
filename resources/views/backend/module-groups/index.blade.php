...
@php
    ...
    $actions = array();
    if($perms->where('controller', 'module-groups')->isNotEmpty() && !empty($perms->where('controller', 'module-groups')->first()))
        $actions = $perms->where('controller', 'module-groups')->first()->actions;
    if(Session::get('is_admin')){
        $actions = ['A', 'M', 'D', 'P'];
    }
@endphp
@if ($perms->where('controller', 'module-groups')->isNotEmpty() || Session::get('is_admin'))
...
@section('content')
        ...
        <div class="modal-dialog modal-dialog-vertical-center" role="document">
            <div class="modal-content bd-0 tx-14">
                ...
                <form id="form-edit" method="post" class="needs-validation" novalidate>
                    ...
                    <div id="modal-edit-body" class="modal-body pd-25"></div>
                    <div class="modal-footer">
                        <button type="submit"
                            class="btn btn-gray tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium">Enregistrer</button>
                        <button type="button"
    ...
    <div class="float-right pd-b-15">
        <a href="#" class="btn btn-yellow btn-with-icon btn-block" id="btn-modal-add">
            <div class="ht-40 justify-content-between text-black">
                <span class="pd-x-15 tx-bold">AJOUTER UN GROUPE</span>
...
@section('js')
    @parent
    <script src="{{ asset('assets/static/js/scripts/module-groups.js') }}"></script>
